...
const slideDeckPattern = /<%\s*if\s*\(\s*activity\.id\s*===\s*['"]profile['"]\s*\)\s*\{\s*%>\r?\n\s*<!-- ── Reto 0 \/ Interactive Slide-Deck Section ── -->[\s\S]*?<\/div>\r?\n\s*<%\s*\}\s*%>/;
if (content.match(slideDeckPattern)) {
    content = content.replace(slideDeckPattern, '');
    console.log('✅ Removed old slide deck block');
} else {
    // Try without comments or slightly different layout
    const altPattern = /<%\s*if\s*\(\s*activity\.id\s*===\s*['"]profile['"]\s*\)\s*\{\s*%>\s*<!--[\s\S]*?<\/div>\s*<%\s*\}\s*%>/;
    const match = content.match(altPattern);
    if (match) {
        content = content.replace(altPattern, '');
        console.log('✅ Removed old slide deck block (alt pattern)');
    } else {
        console.warn('⚠️ Could not find old slide deck block');
    }
}

// 3. Build the accordion content
            ...
            <div class="reto0-accordion" id="reto0Accordion">
                ...
                <div class="accordion-panel" id="panel-1">
                    ...
                    <div class="accordion-content" style="display: block;">
                        <p class="panel-description">🧭 Debes hacer clic en los botones de desplazamiento y en los botones explicativos para ganar puntos o XP.</p>
                        ${slideDeckHtml}
...
// Replace form block
const originalFormContent = fs.readFileSync('scratch/original_form_content.txt', 'utf8');

const formStartStr = '<form id="activityForm">';
const formEndStr = '</form>';

const fullFormReplacement = `${formStartStr}
${accordionHtml}
${originalFormContent}
        <% } %>
${formEndStr}`;

// Let's replace the whole form in EJS content
const formIndexStart = content.indexOf(formStartStr);
const formIndexEnd = content.indexOf(formEndStr) + formEndStr.length;

if (formIndexStart !== -1 && formIndexEnd !== -1) {
    content = content.substring(0, formIndexStart) + fullFormReplacement + content.substring(formIndexEnd);
    console.log('✅ Replaced form content with accordion wrapper');
} else {
    console.error('❌ Could not find form start/end in EJS');
}

// 4. Inject script functions
...
const searchToken = 'window.saveAvatarAndContinue = async function(btnEl) {';
const searchIndex = content.indexOf(searchToken);
if (searchIndex !== -1) {
    const closingToken = 'btn.disabled = false;\r\n                }\r\n            };';
    const altClosingToken = 'btn.disabled = false;\n                }\n            };';
    let endIdx = content.indexOf(closingToken, searchIndex);
    if (endIdx === -1) {
        endIdx = content.indexOf(altClosingToken, searchIndex);
    }
    
    if (endIdx !== -1) {
        const insertionIndex = endIdx + (content.includes('\r\n') ? closingToken.length : altClosingToken.length);
        content = content.substring(0, insertionIndex) + '\n' + scriptInjection + content.substring(insertionIndex);
        console.log('✅ Injected JS functions');
    } else {
        ...
        if (scriptEndIndex !== -1) {
            ...
            console.log('✅ Injected JS functions (fallback before </script>)');
        } else {
            console.error('❌ Could not find closing of saveAvatarAndContinue or script block');
        }
    }
} else {
    console.error('❌ Could not find saveAvatarAndContinue function start');
}

// 5. Save views/activity.ejs
fs.writeFileSync('views/activity.ejs', content, 'utf8');
console.log('🎉 views/activity.ejs successfully updated!');
