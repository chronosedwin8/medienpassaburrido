...
// Clean slideDeckHtml: remove trailing <% } %>
let cleanSlideDeck = slideDeckHtml.trim();
if (cleanSlideDeck.endsWith('<% } %>')) {
    cleanSlideDeck = cleanSlideDeck.slice(0, -7).trim();
    console.log('🧹 Cleaned slideDeckHtml trailing brace');
}

// Clean avatarHtml: remove leading <% if... %> and trailing <% } %>
let cleanAvatar = avatarHtml.trim();
const leadingAvatarMatch = cleanAvatar.match(/^<%\s*if\s*\([\s\S]*?\{\s*%>/);
if (leadingAvatarMatch) {
    cleanAvatar = cleanAvatar.substring(leadingAvatarMatch[0].length).trim();
    console.log('🧹 Cleaned avatarHtml leading condition');
}
if (cleanAvatar.endsWith('<% } %>')) {
    cleanAvatar = cleanAvatar.slice(0, -7).trim();
    console.log('🧹 Cleaned avatarHtml trailing brace');
}

// Clean teachersHtml: remove leading <% if... %> and trailing <% } %>
let cleanTeachers = teachersHtml.trim();
const leadingTeachersMatch = cleanTeachers.match(/^<%\s*if\s*\([\s\S]*?\{\s*%>/);
if (leadingTeachersMatch) {
    cleanTeachers = cleanTeachers.substring(leadingTeachersMatch[0].length).trim();
    console.log('🧹 Cleaned teachersHtml leading condition');
}
if (cleanTeachers.endsWith('<% } %>')) {
    cleanTeachers = cleanTeachers.slice(0, -7).trim();
    console.log('🧹 Cleaned teachersHtml trailing brace');
}

// Clean edilimHtml: remove trailing <% } %>
let cleanEdilim = edilimHtml.trim();
if (cleanEdilim.endsWith('<% } %>')) {
    cleanEdilim = cleanEdilim.slice(0, -7).trim();
    console.log('🧹 Cleaned edilimHtml trailing brace');
}
...
// 5. Test compile BEFORE saving
try {
    ejs.compile(content, { filename: 'views/activity.ejs' });
    console.log('✅ Compilation check PASSED! Saving changes to views/activity.ejs');
    fs.writeFileSync('views/activity.ejs', content, 'utf8');
    console.log('🎉 views/activity.ejs successfully updated and validated!');
} catch (e) {
